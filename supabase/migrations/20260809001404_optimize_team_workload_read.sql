-- Build the team workload response in one authorization-aware, RLS-bypassing
-- helper. The former invoker query re-ran project-access policies for every
-- todo and supporting row, which made 33 concurrent reads CPU-bound.
...
as $$
  ...
  milestone_page as materialized (
    ...
    join accessible_projects as project on project.id = milestone.project_id
