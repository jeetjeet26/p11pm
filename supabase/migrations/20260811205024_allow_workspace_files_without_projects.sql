-- Basecamp originally required every logical file to belong to a project.
-- Workspace files may instead be organization- or client-scoped, so derive
-- blob ownership from files.organization_id when project_id is null.
...
begin
  if new.project_id is null then
    ...
    if new.organization_id is distinct from file_organization_id then
